...
int main()
{
	...
	void* ptrg ;
	int cmpt , imask_button ;
	ptrg = (void*)&cmpt ;

	IOWR_ALTERA_AVALON_PIO_EDGE_CAP(BUTTON_PIO_BASE,0);//RAZ

	alt_irq_register(2,ptrg,ma_fct);
	imask_button = IORD_ALTERA_AVALON_PIO_IRQ_MASK(BUTTON_PIO_BASE);
	IOWR_ALTERA_AVALON_PIO_IRQ_MASK(BUTTON_PIO_BASE , imask_button|0x201);	// Autorisation IT sur les 4 buttons
	imask_button = IORD_ALTERA_AVALON_PIO_IRQ_MASK(BUTTON_PIO_BASE);
